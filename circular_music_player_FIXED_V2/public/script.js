...
const linkedListDiagram = document.getElementById("linkedListDiagram");
...
function renderLinkedListDiagram() {
    if (!linkedListDiagram || songs.length === 0) {
        return;
    }

    linkedListDiagram.innerHTML = "";
    ...
    songs.forEach((song, index) => {
        ...
        node.innerHTML = `${song.title}<br><small>${label}</small>`;
        linkedListDiagram.appendChild(node);

        const arrow = document.createElement("span");
        arrow.textContent = "⇄";
        linkedListDiagram.appendChild(arrow);
    });

    const lastArrow = linkedListDiagram.lastElementChild;
    if (lastArrow && lastArrow.tagName === "SPAN") {
        lastArrow.textContent = "↺";
        lastArrow.title = "Back to Head";
    }
}
